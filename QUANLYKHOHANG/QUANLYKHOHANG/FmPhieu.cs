...
namespace QUANLYKHOHANG
{
    ...
    {
        SqlConnection _cnn = new SqlConnection("Data Source=DESKTOP-85V5Q0S\\VANHIEU;Initial Catalog=QUANLYKHOHANG;Integrated Security=True");
        SQL dt = new SQL();
        SqlCommand cmd;
        Modify modify = new Modify();
        string sql;
        SqlConnection ketnoi;
        string chuoiketnoi = @"Data Source=DESKTOP-85V5Q0S\VANHIEU;Initial Catalog=QUANLYKHOHANG;Integrated Security=True";
        SqlCommand thuchien;
...
//            }
        ...
        private void btnThemN_Click(object sender, EventArgs e)
        {
            ...
            {
                if (dt.ThemPhieu(txtMaPnN.Text, dateNN.Value, txtSL.Text, txtTongTienN.Text, cmMaNvN.Text))
                {
        ...
        private void btnSuaN_Click(object sender, EventArgs e)
        {
            ...
            {
                if (dt.SuaPhieuN(txtMaPnN.Text, dateNN.Value, txtSL.Text, txtTongTienN.Text, cmMaNvN.Text))
                {
        ...
        private void dataPN_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            txtSL.Text = dataPN.SelectedRows[0].Cells[2].Value.ToString();
            txtTongTienN.Text = dataPN.SelectedRows[0].Cells[3].Value.ToString();
            cmMaNvN.SelectedItem = dataPN.SelectedRows[0].Cells[4].Value.ToString();
        }
        ...
        private void dataPX_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            txtSoLuongX.Text = dataPX.SelectedRows[0].Cells[2].Value.ToString();
            txtTongTienX.Text = dataPX.SelectedRows[0].Cells[3].Value.ToString();
            cmMaKh.SelectedItem = dataPX.SelectedRows[0].Cells[4].Value.ToString();
            cmMaNVX.SelectedItem = dataPX.SelectedRows[0].Cells[5].Value.ToString();
        ...
        private void txtTimKiemN_TextChanged(object sender, EventArgs e)
        {
        ...
        private void btnSuaX_Click(object sender, EventArgs e)
        {
            if (MessageBox.Show("Bạn có chắc chắn Sửa không?", "Thông Báo", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1) == DialogResult.Yes)
            {
                ...
                if (dt.SuaPhieuX(txtMaPX.Text, dateNN.Value, txtSoLuongX.Text, txtTongTienX.Text, cmMaKh.Text, cmMaNVX.Text))
                {
                    MessageBox.Show("Sửa Thành Công", "Thông Báo");
                }
            }
        ...
        private void btnThemX_Click(object sender, EventArgs e)
        {
            ...
            if (MessageBox.Show("Bạn có chắc chắn thêm không?", "Thông Báo", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1) == DialogResult.Yes)
            {
                if (dt.ThemPhieuX(txtMaPX.Text, dateNN.Value, txtSoLuongX.Text, txtTongTienX.Text, cmMaKh.Text, cmMaNVX.Text))
                {
                    MessageBox.Show("Thêm Thành Công", "Thông Báo");
                }
            }
